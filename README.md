...
# 3. Queryset
※ 주의 사항  
일반적으로 기존 객체에 자주 영향을 미치는 메소드는 자신(self)을 리턴하지 않도록 하는 것이 Python에서 좋은 사례로 간주된다.   
만약 객체로 접근한 메소드가 자신(self)를 리턴하게 되면 계속 이어서 코드를 작성하게 되고(메소드 체인),   
이 때 특정 버그나 문제가 발생했을 때 찾기 힘들 뿐더러 전체 기능에 문제가 생길 수 있다.  
...
# 3-2. 조회(READ)
	...
	2. all()
		...
		list와 구조는 같지만, 파이썬 자료구조가 아니기 때문에 형변환이나 serializer(직렬화)가 필요하다.

	3. values() --> 골라서 가져올때 사용
		테이블에서 전체 정보를 조회한다.
		QuerySet 객체를  리턴하며, 조회된 객체가 dict 타입으로 들어있다.
		필드 이름을 전달하면 원하는 필드 정보만 가져올 수 있다.
		참조 중인 테이블의 필드를 가져오기 위해서는 '[참조중인 객체명]___[필드명]'으로 작성한다.
	...
	6. exists()
		filter()와 함께 사용해서 filter 조건에 맞는 데이터가 있는 지 조회된다.
	...
	9. first(), last()
		...
		조건에 맞는 QuerySet 결과 중 첫 번째 객체만 가져온다.

		모델명.objects.filter().last()
		조건에 맞는 QuerySet 결과 중 마지막 객체만 가져온다.
